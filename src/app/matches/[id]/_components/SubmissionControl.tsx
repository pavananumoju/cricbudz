...
export default function SubmissionControl({
  ...
  return (
    <div className="flex flex-col gap-3 md:gap-4 transition-all">
      ...
      <div className={cn(
          "flex gap-2 items-center p-2.5 rounded-xl border text-meta font-medium leading-tight italic",
        ...
        <div className="border-t border-border pt-2.5">
          <ul className="flex items-center gap-x-4 gap-y-1.5 overflow-x-auto no-scrollbar pb-1">
            <li className={cn("flex items-center gap-1.5 text-meta font-medium whitespace-nowrap", selectedPlayers.length === 3 ? "text-success" : "text-muted")}>
              <Check size={10} className={selectedPlayers.length === 3 ? "opacity-100" : "opacity-30"} />
              3 Players
            </li>
            <li className={cn("flex items-center gap-1.5 text-meta font-medium whitespace-nowrap", new Set(selectedPlayers.map(p => p.team)).size === 2 ? "text-success" : "text-muted")}>
              <Check size={10} className={new Set(selectedPlayers.map(p => p.team)).size === 2 ? "opacity-100" : "opacity-30"} />
              2 Teams
            </li>
            <li className={cn("flex items-center gap-1.5 text-meta font-medium whitespace-nowrap", mvpId ? "text-success" : "text-muted")}>
