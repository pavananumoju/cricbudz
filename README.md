...
* **System date override** — simulates "today" app-wide (real clock still ticks within that simulated day) for testing lock/completion logic without waiting for real IPL dates. Entered via a native `<input type="date">`, so a malformed value is unrepresentable (previously a free-text field validated by a blocking `alert()`).
...
* Admin sets `{ hideUntilToss: true, date: "YYYY-MM-DD" }` in a single `settings/visibility` document. The date is entered via a native `<input type="date">` (not free text — a typo like `2026-4-18` used to silently arm the toggle against a date that matches nothing, protecting nobody with no warning; the browser's date picker makes that unrepresentable) and is compared against each match's **IST** calendar day (`getMatchDayIST()` in `src/lib/utils.ts`, used by both the match page's toggle check and `GET /api/matches/[matchId]/squads`) — not UTC or the viewer's device timezone. Saving shows a confirmation toast naming the exact day the toggle is now armed (or disarmed) for.
...
* Weeks run **Monday–Sunday, anchored to India Standard Time** (`src/lib/leaderboard.ts`), not the viewer's device timezone — `getWeekRange()` derives day-of-week and boundaries from the match's IST calendar day (via `getMatchDayIST()` in `src/lib/utils.ts`) using pure UTC-offset arithmetic, so results are identical regardless of the browser's local clock. Before this (audit item #8), week boundaries were computed with local `Date` getters (`getDay()`/`setHours()`), so a user whose device wasn't set to IST could see a match land in a different week than the dashboard/fixtures/visibility-toggle agreed it belonged to (all of which independently derive "today" too — dashboard and fixtures now also go through `getMatchDayIST()`). Standings are **computed live** on every page load by querying all scored squads (`totalPoints` set) whose `matchDay` falls in the selected week and summing per user — there's no separate "leaderboard" collection or scheduled rollup job to keep in sync.
* The `userSquads.matchDay` field itself is still written as a **UTC** slice of `matchTimestamp` (`saveUserSquad()` in `dataService.ts`), not IST — `firestore.rules`' `matchesSourceOfTruth()` enforces it as a literal string prefix via regex and has no timezone-aware date function to check an IST day instead. This is safe because IPL matches only ever start at 10:00 or 14:00 UTC (15:30/19:30 IST), so the UTC slice always equals the IST day for real match data (verified against all synced matches) — don't swap this specific write to `getMatchDayIST()` without also updating the rule, since an arbitrary late-UTC-evening timestamp would diverge and get rejected.
