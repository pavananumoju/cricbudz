...
**Styling:** Tailwind v4 (via `@tailwindcss/postcss`, CSS-first `@theme` config — no JS config file), custom fonts wired as CSS variables in `layout.tsx` (`--font-sans` = Inter, `--font-display` = Space Grotesk, `--font-mono` = JetBrains Mono). `globals.css`'s `@theme` also defines a documented legibility floor (audit item #9): `text-meta` (12px) for informational text, `text-micro` (11px) for decorative micro-labels — informational text must not drop below 12px. `src/lib/utils.ts` has `cn()` (clsx + tailwind-merge), `getTeamLogo()`, `getMatchTimeStatus()`, `getMatchDayIST()`, and `shortPlayerName()` (renders "Rohit Sharma" → "R Sharma" for trio chips, disambiguating same-surname players; callers keep the full name on `title`/`aria-label`). `src/components/ui/Sheet.tsx` is a keyboard-accessible modal (`role="dialog"`, focus trap, Escape-to-close).
